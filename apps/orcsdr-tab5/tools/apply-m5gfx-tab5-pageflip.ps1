...
& git -C $repoRoot apply --check --unidiff-zero --directory=$appRelative -- $patch 2>$null
if ($LASTEXITCODE -eq 0) {
  & git -C $repoRoot apply --unidiff-zero --whitespace=nowarn --directory=$appRelative -- $patch
  if ($LASTEXITCODE -ne 0) { throw 'Unable to apply the M5GFX Tab5 page-flip patch.' }
  exit 0
}

& git -C $repoRoot apply --reverse --check --unidiff-zero --directory=$appRelative -- $patch 2>$null
