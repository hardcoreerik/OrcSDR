...
struct TerrainTiming {
  ...
  spectrum_history::RowSpans* row = nullptr;
  uint32_t visible_rows = 0, submissions = 0;
  uint64_t clear_us = 0, draw_us = 0, overlay_us = 0;
...
void allocate_view_buffers(View next) {
  ...
  if (next == View::spectrum3d) {
    ...
    if (!g_terrain_snapshot || !g_terrain_timing.row) {
...
void draw_3d_view() {
  ...
  const int64_t clear_started = esp_timer_get_time();
  g_canvas.fillRect(kPlotX, kPlotY, kPlotW, kPlotH, kBg);
  ...
  g_terrain_timing.clear_us += esp_timer_get_time() - clear_started;
  uint32_t spans = 0;
  g_terrain_timing.visible_rows = 0;
  float floor, ceiling;
  ...
  spectrum_history::render(*g_terrain_snapshot, g_terrain_now,
      ...
      [&](int x, int top, int bottom, uint16_t ridge, uint16_t body, bool fill, bool edge) {
        ++spans;
        g_terrain_timing.row->add(x, top, bottom, ridge, body, fill, edge);
      }, [&] {
        const int64_t draw_started = esp_timer_get_time();
        if (g_terrain_timing.row->count) ++g_terrain_timing.visible_rows;
        g_terrain_timing.row->draw(g_canvas, kPlotX, kPlotY, scale, camera.mode);
        g_terrain_timing.draw_us += esp_timer_get_time() - draw_started;
      });
...
bool initialize(NvsStore* store, AudioSink audio_sink) {
  ...
  g_observer_frame = static_cast<SpectrumFrame*>(
      heap_caps_calloc(1, sizeof(SpectrumFrame), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!g_terrain_timing.samples) g_terrain_timing.samples = static_cast<uint32_t*>(
      heap_caps_calloc(128, sizeof(uint32_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!g_terrain_timing.row) g_terrain_timing.row = static_cast<spectrum_history::RowSpans*>(
      heap_caps_calloc(1, sizeof(spectrum_history::RowSpans), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
...
bool enter(uint8_t origin_screen_value, uint8_t origin_tab_value) {
  ...
  const auto samples = g_terrain_timing.samples;
  const auto row = g_terrain_timing.row;
  g_terrain_timing = {};
  g_terrain_timing.samples = samples;
  g_terrain_timing.row = row;
...
void service_ui(uint32_t now) {
  ...
  const int64_t overlay_started = terrain ? esp_timer_get_time() : 0;
  ...
  if (terrain) g_terrain_timing.overlay_us += esp_timer_get_time() - overlay_started;
  if (!direct_history && frame_drawn) {
    ...
    if (terrain) {
      ...
      if (g_terrain_timing.samples) g_terrain_timing.samples[g_terrain_timing.frames % 128] = esp_timer_get_time() - frame_started;
      ++g_terrain_timing.frames;
      if (g_page_flip_pending) { ++g_presentation_frames; ++g_terrain_timing.submissions; }
...
bool process_command(const char* command, char* response, size_t response_size) {
  ...
  response[0] = '\0';
  if (strcmp(command, "RTL_VIS PERF SOURCE") == 0) {
    if (!g_ui_frame) { snprintf(response, response_size, "RTL_VIS_SOURCE unavailable"); return true; }
    const auto& frame = *g_ui_frame;
    snprintf(response, response_size,
        "RTL_VIS_SOURCE accepted_iq_blocks=%lu analysis_frames=%lu input_drops=%lu analyzed_ms=%lu "
        "center_hz=%lu span_hz=%lu sample_rate=%lu fft=%u history_s=%.0f slices_index=%.0f "
        "elevation=%.1f azimuth=%.1f zoom=%.2f gain=%.2f mesh=%.0f",
        static_cast<unsigned long>(frame.revision), static_cast<unsigned long>(frame.frame_count),
        static_cast<unsigned long>(frame.input_drops), static_cast<unsigned long>(frame.analyzed_ms),
        static_cast<unsigned long>(frame.center_hz), static_cast<unsigned long>(frame.span_hz),
        static_cast<unsigned long>(frame.sample_rate_sps), frame.fft_size,
        value("spectrum3d.history_s"), value("spectrum3d.slices"), value("spectrum3d.elevation_deg"),
        value("spectrum3d.azimuth_deg"), value("spectrum3d.zoom"), value("spectrum3d.z_gain"), value("spectrum3d.mesh_mode"));
    return true;
  }
  if (strcmp(command, "RTL_VIS PERF ID") == 0) {
    snprintf(response, response_size,
        "RTL_VIS_ID renderer=%s sha=%s dirty=%d digest=%s mode=%s target_hz=60 completion_identity=unavailable",
        SPECTRUM3D_RENDERER, SPECTRUM3D_GIT_SHA, SPECTRUM3D_DIRTY, SPECTRUM3D_DIGEST, SPECTRUM3D_BUILD_MODE);
    return true;
  }
  if (strcmp(command, "RTL_VIS PERF CLOCK") == 0) {
    const int64_t start = esp_timer_get_time();
    for (int i = 0; i < 256; ++i) (void)esp_timer_get_time();
    snprintf(response, response_size, "RTL_VIS_CLOCK calls=256 total_us=%lld timing_scope=per_row_not_per_primitive",
             static_cast<long long>(esp_timer_get_time() - start));
    return true;
  }
  if (strcmp(command, "RTL_VIS PERF") == 0) {
    ...
    snprintf(response, response_size,
        ...
        "history_drops=%lu submit_errors=%lu buckets=%u raster=%ux%u spans=%lu quality=%u visible_rows=%lu submissions=%lu target_hz=60 completion_identity=unavailable",
        ...
        static_cast<unsigned long>(g_terrain_timing.visible_rows),
        static_cast<unsigned long>(g_terrain_timing.submissions));
    return true;
  }
  if (strcmp(command, "RTL_VIS PERF TIMING") == 0) {
    const size_t count = g_terrain_timing.samples ? std::min<uint32_t>(128, g_terrain_timing.frames) : 0;
    uint32_t samples[128];
    if (count) std::copy_n(g_terrain_timing.samples, count, samples);
    ...
    snprintf(response, response_size,
        ...
        "geometry_raster_avg=%.0f clear_grid_avg=%.0f draw_avg=%.0f overlay_avg=%.0f submit_avg=%.0f wait_avg=%.0f producer_us_per_s=%.0f samples=%u",
        ...
        (g_terrain_timing.render_us - g_terrain_timing.history_us - g_terrain_timing.clear_us - g_terrain_timing.draw_us) / frames,
        g_terrain_timing.clear_us / frames, g_terrain_timing.draw_us / frames,
        g_terrain_timing.overlay_us / frames,
