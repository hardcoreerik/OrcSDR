...
bool start() {
  ...
  if (netif != ESP_OK) { g_failure_stage = "netif"; g_failure_code = netif; return false; }
...
bool begin_scan() { return g_started && esp_wifi_scan_start(nullptr, false) == ESP_OK; }
int scan_results(ScanResult* results, size_t capacity) {
  uint16_t count = 0;
  if (!g_started || esp_wifi_scan_get_ap_num(&count) != ESP_OK) return -1;
